...
serve(async (req) => {
  // Tambahkan Header CORS (penting untuk produksi dan pengembangan)
  const corsHeaders = {
  ...
  if (!authHeader) {
    return new Response('Unauthorized: Missing Authorization header', { status: 401, headers: corsHeaders }) // Tambahkan headers
  }
  ...
  if (userError || !user || user.id !== '91f184ea-88c1-4b32-adfa-55901ef23db9') { // GANTI INI DENGAN ID ADMIN ANDA!
    ...
    return new Response('Forbidden: Only admin can perform this action', { status: 403, headers: corsHeaders }) // Tambahkan headers
  }
  ...
  try {
    ...
    if (listUsersError) {
      ...
      return new Response(JSON.stringify({ error: listUsersError.message }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders }, // Tambahkan headers
        status: 500,
      })
    }
