...
const AdminLoginPage: React.FC = () => {
  ...
  // Efek untuk mengarahkan admin yang SUDAH login
  useEffect(() => {
    if (isInitialized && !authGlobalLoading && authUser && role) {
      if (role === 'admin' && authUser.id === ADMIN_USER_ID) {
        console.log('ADMIN_LOGIN_PAGE: Already logged in as admin. Redirecting to /admin.');
        navigate('/admin', { replace: true });
      } else {
        // Jika user non-admin terdampar di halaman login admin, arahkan ke profil user biasa
        console.log('ADMIN_LOGIN_PAGE: Logged in as non-admin. Redirecting to /user.');
  ...
  // Tampilkan loading screen jika authStore belum siap
        ...
        <div className="max-w-md w-full space-y-8 p-8 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700">
            ...
            <form className="mt-8 space-y-6" onSubmit={handleAdminLogin}>
                ...
                <div>
                  ...
                  <input id="admin-email-address" name="email" type="email" autoComplete="email" required className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 placeholder-gray-500 text-gray-900 dark:text-white dark:bg-gray-700 focus:outline-none focus:ring-primary-500 focus:border-primary-500 focus:z-10 sm:text-sm" placeholder="Alamat Email Admin" value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div>
                  <label htmlFor="admin-password" className="sr-only">Kata Sandi</label>
                  <input id="admin-password" name="password" type="password" autoComplete="current-password" required className="appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 placeholder-gray-500 text-gray-900 dark:text-white dark:bg-gray-700 focus:outline-none focus:ring-primary-500 focus:border-primary-500 focus:z-10 sm:text-sm" placeholder="Kata Sandi Admin" value={password} onChange={(e) => setPassword(e.target.value)} />
                </div>
