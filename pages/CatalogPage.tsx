...
const CatalogPage: React.FC<CatalogPageProps> = ({ searchTerm }) => {
  ...
  const uniqueKategori = useMemo(() => [...new Set(manuscripts.map(m => m.kategori_ilmu_pesantren))], [manuscripts]);
  const uniqueBahasa = useMemo(() => [...new Set(manuscripts.flatMap(m => m.bahasa.split(',').map(b => b.trim())))], [manuscripts]);
  const uniqueAksara = useMemo(() => [...new Set(manuscripts.flatMap(m => m.aksara.split(',').map(a => a.trim())))], [manuscripts]);
  ...
  const filteredManuscripts = useMemo(() => {
    return manuscripts.filter(ms => {
      ...
      const searchMatch = !searchTerm ||
        ms.judul_dari_tim.toLowerCase().includes(searchTermLower) ||
        ms.pengarang.toLowerCase().includes(searchTermLower) ||
        ms.deskripsi_umum.toLowerCase().includes(searchTermLower);

      const kategoriMatch = filters.kategori === 'all' || ms.kategori_ilmu_pesantren === filters.kategori;
      const bahasaMatch = filters.bahasa === 'all' || ms.bahasa.split(',').map(b => b.trim()).includes(filters.bahasa);
      const aksaraMatch = filters.aksara === 'all' || ms.aksara.split(',').map(a => a.trim()).includes(filters.aksara);
  ...
  return (
    <div>
      <h1 className="text-4xl font-bold font-serif mb-8 text-center">Katalog Manuskrip</h1>
      ...
      {totalPages > 1 && (
        <div className="mt-12 flex justify-center items-center space-x-4">
          <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-md disabled:opacity-50 disabled:cursor-not-allowed bg-gray-200 dark:bg-gray-700">
            <ChevronLeftIcon className="h-5 w-5" />
          </button>
          <span>Halaman <strong>{currentPage}</strong> dari <strong>{totalPages}</strong></span>
          <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-md disabled:opacity-50 disabled:cursor-not-allowed bg-gray-200 dark:bg-gray-700">
